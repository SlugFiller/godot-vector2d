...
@export var stroke_width : float = 1.0:
	set(width):
		...
		stroke_width = width
		set_dirty()
@export var miter_limit : float = 4.0:
	set(limit):
		if limit < 0.0:
			return
		miter_limit = limit
		set_dirty()
@export_enum("Arcs", "Bevel", "Miter", "Miter Clip", "Round") var join_type : int = 2:
	set(join):
		if join < 0 || join > 4:
			return
		join_type = join
		set_dirty()
@export_enum("Butt", "Round", "Square") var cap_type : int = 0:
	set(cap):
		if cap < 0 || cap > 2:
			return
		cap_type = cap
...
func _check_shape_update() -> void:
	...
	if new_shape_node == shape_node:
		return
	if shape_node && shape_node.shape_changed.is_connected(Callable(self, "set_dirty")):
		shape_node.shape_changed.disconnect(Callable(self, "set_dirty"))
	shape_node = new_shape_node
	if shape_node && !shape_node.shape_changed.is_connected(Callable(self, "set_dirty")):
		shape_node.shape_changed.connect(Callable(self, "set_dirty"))
	set_dirty()
	...
	if segments[0][2] & 1:
		if join.size() > 1:
			if join[0].is_equal_approx(join[4]):
				...
				var tangent : Vector2 = (join[4]-join[0]).normalized().orthogonal()
		...
		match cap_type:
			1:
				if join.size() <= 1:
					...
					dest.append([SEGMENT_TYPE.ARC, join[0]-join[1].orthogonal()*(stroke_width*0.5), join[0], join[0]-join[1]*(stroke_width*0.5), join[0]-join[1]*(stroke_width*0.5)])
			2:
				if join.size() <= 1:
					dest.append([SEGMENT_TYPE.START, join[0], 1])
				else:
					dest.append([SEGMENT_TYPE.LINEAR, join[0]+(join[1]-join[1].orthogonal())*(stroke_width*0.5)])
					dest.append([SEGMENT_TYPE.LINEAR, join[0]-(join[1]+join[1].orthogonal())*(stroke_width*0.5)])
			_:
				if join.size() <= 1:
					dest.append([SEGMENT_TYPE.START, join[0], 1])
...
func _offset_linear(join : Array, end : Vector2, dest : Array) -> void:
	...
	if join[0].is_equal_approx(end):
		return
	var tangent : Vector2 = (end-join[0]).normalized().orthogonal()
	if join.size() > 1:
...
func _offset_quadric(join : Array, control : Vector2, end : Vector2, dest : Array, max_depth : int) -> void:
	...
	if start.is_equal_approx(control) && start.is_equal_approx(end):
		return
	var tangent_start : Vector2 = ((end-start) if start.is_equal_approx(control) else (control-start)).normalized().orthogonal()
	var tangent_end : Vector2 = ((end-start) if end.is_equal_approx(control) else (end-control)).normalized().orthogonal()
	var tangent_mid : Vector2 = ((control-start) if end.is_equal_approx(start) else ((end-start).orthogonal())).normalized()
	...
	if max_depth > 0:
		...
		for i in TEST_DIVISIONS-2:
			...
			b += ((b-a1) if !b.is_equal_approx(a1) else (b-start)).orthogonal().normalized()*stroke_width*0.5
...
func _offset_cubic(join : Array, control1 : Vector2, control2 : Vector2, end : Vector2, dest : Array, max_depth : int) -> void:
	...
	tangent_start = tangent_start.normalized().orthogonal()
	...
	tangent_end = tangent_end.normalized().orthogonal()
	var tangent_mid : Vector2 = ((end+control2)-(start+control1)).normalized().orthogonal()
	var start_shift : Vector2 = start+tangent_start*(stroke_width*0.5)
	...
	if max_depth > 0:
		...
		for i in TEST_DIVISIONS-2:
			...
			c += ((c-b1) if !c.is_equal_approx(b1) else ((c-a1) if !c.is_equal_approx(a1) else (c-start))).orthogonal().normalized()*stroke_width*0.5
			var u1 : Vector2 = start_shift+t*(control1_shift-start_shift)
			var u2 : Vector2 = control1_shift+t*(control2_shift-control1_shift)
			var u3 : Vector2 = control2_shift+t*(end_shift-control2_shift)
			var v1 : Vector2 = u1+t*(u2-u1)
...
func _offset_arc(join : Array, center : Vector2, axis : Transform2D, start : Vector2, end : Vector2, dest : Array, max_depth : int) -> void:
	...
	var tangent_start : Vector2 = axis.basis_xform(control-start).orthogonal().normalized()
	var tangent_end : Vector2 = axis.basis_xform(end-control).orthogonal().normalized()
	var mid : Vector2 = (start+end).normalized()
	var tangent_mid : Vector2 = axis.basis_xform(-mid.orthogonal()).orthogonal().normalized()
	...
	if max_depth > 0:
		...
		for i in TEST_DIVISIONS-2:
			...
			var p : Vector2 = (center+axis.basis_xform(mid1))-axis.basis_xform(mid1.orthogonal()).normalized().orthogonal()*(stroke_width*0.5)
			var u1 : Vector2 = start_shift+t*(control1_shift-start_shift)
			var u2 : Vector2 = control1_shift+t*(control2_shift-control1_shift)
			var u3 : Vector2 = control2_shift+t*(end_shift-control2_shift)
			var v1 : Vector2 = u1+t*(u2-u1)
...
func _join(dest : Array, point : Vector2, tangent_start : Vector2, curvature_start : float, allow_type : bool, tangent_end : Vector2, curvature_end : float) -> void:
	if tangent_start.is_equal_approx(tangent_end):
		return
	if tangent_start.orthogonal().dot(tangent_end) > 0.0:
	...
	match join_type if allow_type else 4:
		0:
			...
			if curvature_start*stroke_width > 2.0 || curvature_end*stroke_width > 2.0:
				dest.append([SEGMENT_TYPE.ARC, point+tangent_end.orthogonal()*(stroke_width*0.5), point, point+tangent_end*(stroke_width*0.5), point+tangent_end*(stroke_width*0.5)])
				return
			var tangent_mid : Vector2 = tangent_start+tangent_end
			if tangent_mid.length_squared() < 1.0:
				tangent_mid = (tangent_end-tangent_start).orthogonal()
			...
			if is_zero_approx(curvature_start):
				...
				var line_dir : Vector2 = -tangent_start.orthogonal()
				...
				var miter_radius : float = tangent_mid.orthogonal().dot(intersection-point)
				if is_zero_approx(miter_radius):
					...
					var miter_center : Vector2 = point+tangent_mid.orthogonal()*miter_radius
					if abs(tangent_mid.orthogonal().angle_to(miter_center-intersection)*miter_radius) > miter_limit*stroke_width*0.5:
						miter_clip = true
						miter_point = miter_center-tangent_mid.orthogonal().rotated(-miter_limit*stroke_width*0.5/miter_radius)*miter_radius
				...
				if miter_clip:
					...
					b = -miter_tangent.orthogonal().dot(miter_point-center)
					c = radius*radius-miter_point.distance_squared_to(center)
					d = sqrt(b*b-c)
					var intersection2 : Vector2 = miter_point-miter_tangent.orthogonal()*((b+d) if b < d else (b-d))
					dest.append([SEGMENT_TYPE.LINEAR, intersection1])
					dest.append([SEGMENT_TYPE.LINEAR, intersection2])
					dest.append([SEGMENT_TYPE.ARC, center+tangent_end.orthogonal()*radius, center, point+tangent_end*stroke_width*0.5, point+tangent_end*stroke_width*0.5])
				else:
			...
			if is_zero_approx(curvature_end):
				...
				var line_dir : Vector2 = tangent_end.orthogonal()
				var b : float = line_dir.dot(line_start-center)
				var c : float = line_start.distance_squared_to(center)-radius*radius
				var d : float
				if b*b < c:
				...
				if miter_clip:
					...
					c = radius*radius-miter_point.distance_squared_to(center)
					d = sqrt(b*b-c)
					var intersection1 : Vector2 = miter_point+miter_tangent.orthogonal()*((b+d) if b < d else (b-d))
					...
					dest.append([SEGMENT_TYPE.ARC, center+tangent_start.orthogonal()*radius, center, point+tangent_start*stroke_width*0.5, intersection1])
					dest.append([SEGMENT_TYPE.LINEAR, intersection2])
					dest.append([SEGMENT_TYPE.LINEAR, point+tangent_end*stroke_width*0.5])
				else:
					dest.append([SEGMENT_TYPE.ARC, center+tangent_start.orthogonal()*radius, center, point+tangent_start*stroke_width*0.5, intersection])
					dest.append([SEGMENT_TYPE.LINEAR, point+tangent_end*stroke_width*0.5])
				return
			...
			if center1.distance_squared_to(center2) > (radius1+radius2)*(radius1+radius2):
				...
				var line_dir : Vector2 = (center2-line_start).orthogonal().normalized() if center1.is_equal_approx(line_start) else ((line_start-center1).orthogonal().normalized())
				var d : float = sqrt(radius1*radius1-line_start.distance_squared_to(center1))
				intersection = line_start+d*line_dir
				if tangent_end.orthogonal().dot(point+tangent_end*stroke_width*0.5-intersection) > 0:
					intersection = line_start-d*line_dir
				else:
					var intersection2 : Vector2 = line_start-d*line_dir
					if tangent_end.orthogonal().dot(point+tangent_end*stroke_width*0.5-intersection2) < 0 && intersection2.distance_squared_to(point+tangent_end*stroke_width*0.5) < intersection.distance_squared_to(point+tangent_end*stroke_width*0.5):
			...
			if is_zero_approx(miter_radius):
				...
				if abs(tangent_mid.orthogonal().angle_to(miter_center-intersection)*miter_radius) > miter_limit*stroke_width*0.5:
					...
					miter_tangent = tangent_mid.rotated(-miter_limit*stroke_width*0.5/miter_radius)
			if miter_clip:
				var b : float = miter_tangent.orthogonal().dot(miter_point-center1)
				var c : float = radius1*radius1-miter_point.distance_squared_to(center1)
				var d : float = sqrt(b*b-c)
				var intersection1 : Vector2 = miter_point+miter_tangent.orthogonal()*((b+d) if b < d else (b-d))
				b = -miter_tangent.orthogonal().dot(miter_point-center2)
				c = radius2*radius2-miter_point.distance_squared_to(center2)
				d = sqrt(b*b-c)
				var intersection2 : Vector2 = miter_point-miter_tangent.orthogonal()*((b+d) if b < d else (b-d))
				dest.append([SEGMENT_TYPE.ARC, center1+tangent_start.orthogonal()*(radius1 if tangent_start.orthogonal().dot(intersection1-center1) < 0 else -radius1), center1, point+tangent_start*stroke_width*0.5, intersection1])
				dest.append([SEGMENT_TYPE.LINEAR, intersection2])
				dest.append([SEGMENT_TYPE.ARC, center2+tangent_end.orthogonal()*(radius2 if tangent_end.orthogonal().dot(center2-intersection2) < 0 else -radius2), center2, point+tangent_end*stroke_width*0.5, point+tangent_end*stroke_width*0.5])
			else:
				dest.append([SEGMENT_TYPE.ARC, center1+tangent_start.orthogonal()*(radius1 if tangent_start.orthogonal().dot(intersection-center1) < 0 else -radius1), center1, point+tangent_start*stroke_width*0.5, intersection])
				dest.append([SEGMENT_TYPE.ARC, center2+tangent_end.orthogonal()*(radius2 if tangent_end.orthogonal().dot(center2-intersection) < 0 else -radius2), center2, point+tangent_end*stroke_width*0.5, point+tangent_end*stroke_width*0.5])
		2:
...
func _miter(dest : Array, point : Vector2, tangent_start : Vector2, tangent_end : Vector2, clip : bool) -> void:
		...
		if miter_limit*det >= 1:
			...
			det = (miter_limit-tangent_start.dot(tangent_mid))/tangent_start.orthogonal().dot(tangent_mid)
			dest.append([SEGMENT_TYPE.LINEAR, point+(tangent_start+tangent_start.orthogonal()*det)*stroke_width*0.5])
			dest.append([SEGMENT_TYPE.LINEAR, point+(tangent_end-tangent_end.orthogonal()*det)*stroke_width*0.5])
			dest.append([SEGMENT_TYPE.LINEAR, point+tangent_end*(stroke_width*0.5)])
